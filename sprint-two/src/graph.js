...
// Add a node to the graph, passing in the node's value.
  ...
  removeNode(node) { 
    for (var key in this.nodes) {
      this.nodes[key] = this.nodes[key].filter((value) => value !== node
      );
    }
    delete this.nodes[JSON.stringify(node)];
    //complexity quadratic O(n^2)
  ...
  removeEdge(fromNode, toNode) {
    this.nodes[fromNode] = this.nodes[fromNode].filter((value) => value !== toNode
    );
    this.nodes[toNode] = this.nodes[toNode].filter((value) => value !== fromNode
    );
    //complexity linear O(n)
  };
